...
class SenseHat(object):
    ...
    def set_pixels(self, pixel_list):
        ...
        for index, pix in enumerate(pixel_list):
            if len(pix) != 3:
                raise ValueError(
                    "Pixel at index %d is invalid. Pixels must contain 3 elements:",
            ...
            for element in pix:
                if element > 255 or element < 0:
                    raise ValueError(
                        "Pixel at index %d is invalid. Pixel elements must be between",
                        "0 and 255" % index,
                    )
    ...
    def get_compass(self):
        ...
        if isinstance(orientation, dict) and "yaw" in orientation.keys():
